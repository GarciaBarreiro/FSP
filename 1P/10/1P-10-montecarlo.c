...
long int montecarlo(long int num_iter, int x_range, int y_range) {
    ...
    for (int i = 0; i < num_iter; i++) {
        ...
        y = (rand() % y_range)/(double)100000000;
...
int main(int argc, char *argv[]) {
    int node = 0, npes;
    int x_range = 6, y_range = 1666666666;  // x is [-3,3]; y is [0,10]
    ...
    if (!node){
        // agora area 60u, non 100
        double integral = total / (double)(num_iter * npes);   // sigue sen estar ben
        double pi = integral/3;
